...
const TBScreen = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const tabletProducts = response.data.products.filter(product => product.category === 'Máy tính bảng');
        setProducts(tabletProducts);
        setFilteredProducts(tabletProducts); // Gán filteredProducts với tất cả sản phẩm máy tính bảng

        // Lấy danh sách tất cả các nhãn hiệu
        const uniqueBrands = [...new Set(tabletProducts.map(item => item.thuongHieu))];
  ...
  const handleSort = (order) => {
    ...
    setVisibleSortMenu(false); // Đóng menu sắp xếp sau khi chọn
  };
  ...
  return (
    <Provider>
      <View style={styles.container}>
        <Text style={styles.header}>Danh mục Máy tính bảng</Text>
...
const styles = StyleSheet.create({
  ...
  // Để căn giá sang trái như trong TVScreen
  productPrice: {
    fontSize: 14,
    color: '#e74c3c',
    fontWeight: 'bold',
  ...
  // Căn chỉnh giá trong container
  priceContainer: {
    flexDirection: 'row',
    justifyContent: 'flex-start',  // Căn giá gốc và phần trăm giảm về trái
    width: '100%',
    marginBottom: 5,
  },

  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const tvProducts = response.data.products.filter(product => product.category === 'TV');
        setProducts(tvProducts);
        setFilteredProducts(tvProducts);

        const uniqueBrands = [...new Set(tvProducts.map(item => item.thuongHieu))];
  ...
  return (
    <Provider>
      <View style={styles.container}>
        <Text style={styles.header}>Danh mục TV</Text>

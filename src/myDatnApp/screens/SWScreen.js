...
import { StyleSheet, Text, View, FlatList, Image, TextInput, Pressable, Alert } from 'react-native';
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const smartWatchProducts = response.data.products.filter(product => product.category === 'Smart Watch');
        setProducts(smartWatchProducts);
        setFilteredProducts(smartWatchProducts);

        const uniqueBrands = [...new Set(smartWatchProducts.map(item => item.thuongHieu))];
  ...
  return (
    <Provider>
      <View style={styles.container}>
        <Text style={styles.header}>Danh mục Smart Watch</Text>
...
const styles = StyleSheet.create({
  ...
  productPrice: {
    ...
    fontWeight: 'bold',
    marginBottom: 5,
    textAlign: 'left',  // Căn giá sang trái
  ...
  priceRow: {
    flexDirection: 'row',
    justifyContent: 'flex-start',  // Căn giá bên trái
    width: '100%',
    marginBottom: 5,
  },
});

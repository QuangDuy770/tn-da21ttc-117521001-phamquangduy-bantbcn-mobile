  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const accessoryProducts = response.data.products.filter(product => product.category === 'Phụ kiện');
        setProducts(accessoryProducts);
        setFilteredProducts(accessoryProducts);

        // Lấy danh sách tất cả các nhãn hiệu
        const uniqueBrands = [...new Set(accessoryProducts.map(item => item.thuongHieu))];
  ...
  return (
    <Provider>
      <View style={styles.container}>
        <Text style={styles.header}>Danh mục Phụ kiện</Text>
        ...
        {/* Thanh lọc thương hiệu và sắp xếp ở dưới cùng */}
        <View style={styles.searchSortContainer}>
